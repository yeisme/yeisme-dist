...
**RC isolation (operational).** A Scaena prerelease never advances the stable catalog `latest` (verify-policy products never fall back to a prerelease), never receives a success receipt (policy eligibility excludes prereleases), and never renders public manifests. `scripts/generate-package-manifests.sh --rc-tag scaena/v<version> --rc-release <github-release.json> --rc-root <dir>` renders the six RC manifests into a throwaway `Tap/Casks` + `Bucket/bucket` layout only; `scripts/rc-channel.sh create|destroy` wraps the lifecycle, snapshots the stable generated surface on create and proves it byte-identical on destroy. CI renders and destroys the channel via `.github/workflows/scaena-rc-channel.yml` (read-only, `if: always()` cleanup); the channel must never outlive its job.

**Channel rollback (operational).** When promoted Scaena manifests prove defective, `scripts/rollback-manifests.sh --product scaena --version vX.Y.Z --reason "..."` appends an immutable failure record at `receipts/scaena/failures/vX.Y.Z-manifest-rollback.json`, demotes catalog `latest`/`verified_latest` past the defective version, regenerates every manifest from the demoted catalog (the generator removes the stale three-package group when the eligible latest falls back below v0.4), and refreshes the README table. `write_catalog` computes the same demoted `latest` from the failure record on every regeneration, so the next sync converges on the rollback instead of re-promoting. Success receipts and mirrored release bytes are never rewritten; the fix ships as the next patch release and the upstream tag is never reused.
